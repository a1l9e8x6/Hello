...
PopupLayer* PopupLayer::create(CCSprite* spritebg,CCScale9Sprite* sprite9){
    PopupLayer* ml = PopupLayer::create();
    ml->setSpriteBackGround( spritebg);
    ml->setSprite9BackGround(sprite9);
    return ml;
}
...
void PopupLayer::setTitle(const char*title,int fontsize,ccColor3B color)
{
...
bool PopupLayer::addButton(CCSprite*normalsprite, CCSprite *selectedsprite, const char *title, int tag){
    CCSize winSize = CCDirector::sharedDirector()->getWinSize();
    CCPoint pCenter = ccp(winSize.width / 2, winSize.height / 2);
    
    // 创建图片菜单按钮
	CCMenuItemSprite* meusprite = CCMenuItemSprite::create(normalsprite,selectedsprite,NULL,this, menu_selector(PopupLayer::buttonCallback));
    meusprite->setTag(tag);
    meusprite->setPosition(pCenter);
    
    // 添加文字说明并设置位置
    CCSize imenu = meusprite->getContentSize();
    CCLabelTTF* ttf = CCLabelTTF::create(title, "", 20);
    ttf->setColor(ccc3(0, 0, 0));
    ttf->setPosition(ccp(imenu.width / 2, imenu.height / 2));
    meusprite->addChild(ttf);
    
    getMenuButton()->addChild(meusprite);
    return true;
}
